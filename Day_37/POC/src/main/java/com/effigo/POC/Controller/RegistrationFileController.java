...
public class RegistrationFileController {
    ...
    public void init() {
        ...
        this.filesize = serverProperties.getFileSize();
    }
    @Autowired
    ...
    public ResponseEntity<String> uploadFile(@RequestParam("file") MultipartFile file) {
        try {
            ...
            String s3Url = s3Service.uploadRegistrationFile(uploadFile, bucketName);
            long timestamp = Instant.now().toEpochMilli();
            String s3Key="effigo/registration/"+uploadFile.getName();
            log.info("s3Key: " + s3Key);

            registrationService.saveRegFromExcel(file,s3Key);
    ...
    public ResponseEntity<String> saveRegistration(@RequestBody RegistrationDTO registrationDTO) {
        try
        {

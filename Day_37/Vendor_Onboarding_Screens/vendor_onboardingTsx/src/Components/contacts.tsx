...
interface EscalationDetailsModalProps {
  open: boolean;
  handleClose: () => void;
  handleSave: (data: EscalationDetails) => void;
}
...
const EscalationDetailsModal: React.FC<EscalationDetailsModalProps> = ({
  ...
  const [formData, setFormData] = useState<EscalationDetails>({
    level1: { name: "", designation: "", contactNumber: "", emailId: "" },
    level2: { name: "", designation: "", contactNumber: "", emailId: "" },
    level3: { name: "", designation: "", contactNumber: "", emailId: "" },
  });

  const handleChange = (
    level: keyof EscalationDetails,
  ...
  const renderLevelFields = (
    ...
    return (
      <Box sx={{ mb: 3 }}>
        <Typography
          variant="subtitle1"
          sx={{ fontWeight: 500, mb: 1.5, color: "#4c5c68" }}
        >
          Level {levelNumber}
        </Typography>
...
// Example usage
const EscalationDetailsComponent: React.FC = () => {
  const [open, setOpen] = useState<boolean>(false);

  const handleOpen = () => setOpen(true);
  const handleClose = () => setOpen(false);

  const handleSave = (data: EscalationDetails) => {
